...
class DateTimeFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_transformed['transaction_day_of_month'] = X_transformed[self.datetime_col].dt.day
...
class CustomWOEEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        for col in self.variables:
            ...
            woe_table = temp_df.groupby('feature')['target'].agg(
                total_count='count',
                good_count=lambda x: (x == 0).sum(),
                bad_count=lambda x: (x == 1).sum()
            ).reset_index()

            woe_table['good_rate'] = (woe_table['good_count'] + epsilon) / (good + epsilon)
    ...
    def transform(self, X):
        if isinstance(X, np.ndarray):
            if self.variables is None:
                raise ValueError("Cannot convert ndarray to DataFrame without column names.")
...
def run_preprocessing(df_raw, target_column='FraudResult'):
    ...
    numerical_features = [
        ...
        'amount_sum_accountid', 'amount_mean_accountid', 'amount_std_accountid',
        'value_sum_accountid', 'value_mean_accountid', 'value_std_accountid',
...
if __name__ == '__main__':
    ...
    df_raw_test = pd.DataFrame({
        ...
        'ProductCategory': np.random.choice(['Bills', 'Airtime', 'Data', 'Other'], n_rows),
