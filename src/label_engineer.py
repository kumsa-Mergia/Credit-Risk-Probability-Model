...
class RiskLabelEngineer:
    ...
    def compute_rfm(self):
        ...
        self.transactions_df["TransactionStartTime"] = pd.to_datetime(self.transactions_df["TransactionStartTime"])
        
        # Define snapshot date (1 day after last transaction)
        snapshot_date = self.transactions_df["TransactionStartTime"].max() + pd.Timedelta(days=1)
    ...
    def cluster_customers(self, n_clusters=3, random_state=42):
        ...
        self.high_risk_cluster = cluster_summary.sort_values(by=["Frequency", "Monetary", "Recency"], ascending=[True, True, False]).index[0]

        self.rfm_df["is_high_risk"] = (self.rfm_df["Cluster"] == self.high_risk_cluster).astype(int)

    def merge_target(self):
        # Merge is_high_risk into main_df
        self.main_df = self.main_df.merge(
            self.rfm_df[["CustomerId", "is_high_risk"]], 
            on="CustomerId", 
            how="left"
        ).fillna({"is_high_risk": 0})  # Default to 0 if missing
